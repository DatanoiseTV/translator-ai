...
function getDefaultCacheFilePath(): string {
  return path.join(getCacheDirectory(), 'translation-cache.json');
}

// --- HELPER FUNCTIONS ---
const hashString = (text: string): string => createHash('sha256').update(text).digest('hex');
...
function flattenObjectWithPaths(obj: JsonValue, currentPath: string = '', result: Map<string, string> = new Map()): Map<string, string> {
...
function unflattenObject(flatMap: Map<string, string>): JsonObject {
